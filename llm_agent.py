...
"""
        ...
        # update_profile - ТОЛЬКО если НЕ обновлялся ранее и есть история
        if not has_updated_profile and not is_brand_new and warmup_stage <= 3:
            # Формируем инструкцию с учетом персоны
            profile_instruction = f"""
{action_num}. update_profile (доступно только на стадиях 1-3):
   {{"action": "update_profile", "first_name": "...", "last_name": "...", "bio": "...", "reason": "..."}}
   
   ⚠️ КРИТИЧЕСКИ ВАЖНО - ВЕДИ СЕБЯ КАК РЕАЛЬНЫЙ ЧЕЛОВЕК:
   """
            
            # Если есть persona - даём ВАРИАНТЫ на основе реального имени
            if persona_data:
                persona_name = persona_data.get('generated_name', '')
                # Извлекаем имя (без фамилии)
                first_name_only = persona_name.split()[0] if persona_name else ''
                
                profile_instruction += f"""
   - Твоё реальное имя: {persona_name}, но ты НЕ ОБЯЗАН его использовать!
   - Реальные люди часто НЕ ставят своё настоящее имя в Telegram:
     * Можешь поставить только имя: {{"first_name": "{first_name_only}"}}
     * Можешь придумать ник/псевдоним: {{"first_name": "Клеопатра"}} или {{"first_name": "dark_angel"}}
     * Можешь использовать уменьшительное: {{"first_name": "Маша"}} (если ты Мария)
     * Можешь оставить вообще пустым: {{"first_name": ""}}
   - РАЗНООБРАЗЬ! Не всегда один и тот же паттерн!
   """
            else:
                profile_instruction += """
   - НЕ ВСЕГДА нужно полное имя+фамилия! Реальные люди разные:
     * Некоторые ставят только имя: {{"first_name": "Саша"}}
     * Некоторые ставят ник: {{"first_name": "dark_phoenix"}}, {{"first_name": "luna"}}
     * Некоторые используют псевдонимы: {{"first_name": "Аврора"}}, {{"first_name": "Феникс"}}
     * Некоторые ставят короткие ники: {{"first_name": "Alex"}}, {{"first_name": "K"}}
     * Некоторые вообще оставляют пустым: {{"first_name": "", "last_name": ""}}
     * Редко кто ставит ПОЛНОЕ имя+фамилию сразу
   
   🚫 ЗАПРЕЩЕНО использовать типичные шаблонные сочетания:
     ❌ "Алексей Иванов" - слишком типично!
     ❌ "Иван Петров" - слишком типично!
     ❌ "Екатерина Смирнова" - слишком типично!
     ❌ Любые "Имя + Популярная русская фамилия" - это ПАЛЕВО!
   ...
   """
            ...
            profile_instruction += """
   - Bio тоже НЕ ОБЯЗАТЕЛЬНО! Многие оставляют пустым или ставят короткий текст (5-15 слов)
   - НЕ делай шаблонно! Варьируй: иногда только имя, иногда ник, иногда вообще пропусти
   - ⚠️ Используй ТОЛЬКО ОДИН РАЗ ЗА ВСЁ ВРЕМЯ!
   """
            
            actions_list.append(profile_instruction)
            action_num += 1
            logger.info(f"✅ update_profile ДОСТУПЕН для {session_id} (не использовался ранее)")
        elif has_updated_profile:
            logger.info(f"🚫 update_profile ЗАБЛОКИРОВАН для {session_id} (уже использовался)")
        elif is_brand_new:
            logger.info(f"🚫 update_profile ЗАБЛОКИРОВАН для {session_id} (нет истории активности)")
