...
```
┌─────────────────────────────────────────────────────────────────────┐
│  ФАЗА 1: Контролируемая среда (между своими ботами) ✅ ЗАВЕРШЕНО     │
│  ├── 1.1 Вспомогательные аккаунты (спамблок) ✅                      │
│  ├── 1.2 ЛС диалоги между ботами ✅                                  │
│  ├── 1.3 Приватные группы ботов ✅                                   │
│  └── 1.4 Проверка входящих DM и ответы ✅                            │
├─────────────────────────────────────────────────────────────────────┤
...
#### Фаза 1.4: Проверка входящих DM и ответы (ЗАВЕРШЕНО 2026-01-08)

**Цель:** LLM видит непрочитанные входящие сообщения и может на них ответить

Реализовано:
- [x] `get_pending_incoming_dms()` в database.py - получает активные разговоры с непрочитанными сообщениями
- [x] Динамическое добавление pending DM в промпт LLM
- [x] Новое действие `reply_to_dm` для ответа на входящие сообщения
- [x] Валидация `reply_to_dm` в llm_agent.py (conversation_id, message)
- [x] Обработчик `_reply_to_dm()` в executor.py:
  - Получает conversation и peer из БД
  - Отправляет сообщение через Telegram API
  - Записывает сообщение в conversation_messages
  - Обновляет счётчик сообщений

**Как это работает:**
```
При каждом цикле прогрева:
1. Проверяем get_pending_incoming_dms() для аккаунта
2. Если есть непрочитанные DM - добавляем в промпт:
   📬 НЕПРОЧИТАННЫЕ СООБЩЕНИЯ В ЛС:
   - От Арина (conv_id=7): "Привет! Как дела?..."
3. LLM может выбрать reply_to_dm с conversation_id и текстом ответа
4. executor отправляет ответ и записывает в БД
```
...
#### Фаза 2.1: Поиск тематических чатов (ЗАВЕРШЕНО)

Расширен `search_agent.py` для приоритизации публичных групп:
- [x] Поисковые запросы приоритизируют "группа", "чат", "обсуждение"
- [x] LLM ранжирует группы выше каналов (can write > read-only)
- [x] Фильтрация по chat_type: group/supergroup

#### Фаза 2.2: Анализ контекста (ЗАВЕРШЕНО)

Создан `chat_context_analyzer.py`:
- [x] `analyze_chat_context()` - LLM анализирует последние сообщения
- [x] Определение темы, тона, уместности ответа
- [x] `should_participate_in_chat()` - оценка релевантности чата для персоны
- [x] Confidence threshold (0.6) для принятия решения об ответе

#### Фаза 2.3: Контекстные ответы (ЗАВЕРШЕНО)

Улучшен `executor.py` - action `reply_in_chat`:
- [x] Автоматический анализ контекста перед ответом
- [x] LLM генерирует контекстный ответ на основе истории чата
- [x] Проверка spam-паттернов в генерируемом тексте
- [x] reply_text опционален - система генерирует сам

#### Фаза 2.4: Ответы живым людям (ЗАВЕРШЕНО)

Создан `real_chat_engine.py`:
- [x] `RealChatEngine` - координатор участия в реальных чатах
- [x] Интеграция в scheduler (каждый цикл проверки)
- [x] Daily message limits (3 сообщения на группу в день)
- [x] Минимальная стадия для участия: stage 8+

### Архитектура Phase 2

```
┌─────────────────────────────────────────────────────────────────────┐
│                      SCHEDULER (каждый цикл)                        │
│                              │                                       │
│                              ▼                                       │
│         ┌──────────────────────────────────────────┐                │
│         │           RealChatEngine                  │                │
│         │  • Находит eligible accounts (stage 8+)   │                │
│         │  • Получает chats_for_participation       │                │
│         │  • Координирует отправку                  │                │
│         └────────────────┬─────────────────────────┘                │
│                          │                                           │
│                          ▼                                           │
│         ┌──────────────────────────────────────────┐                │
│         │         ChatContextAnalyzer              │                │
│         │  • Fetch последних 30 сообщений          │                │
│         │  • LLM анализ: тема, тон, уместность     │                │
...
│         ┌──────────────────────────────────────────┐                │
│         │              Telegram API                │                │
│         │  • send_message() с контекстным текстом  │                │
│         │  • Обновление статистики в БД            │                │
│         └──────────────────────────────────────────┘                │
└─────────────────────────────────────────────────────────────────────┘
```

### Новые таблицы БД (Phase 2)

```sql
-- Кеш сообщений из реальных чатов
CREATE TABLE real_chat_messages (
    id INTEGER PRIMARY KEY,
...
-- Статистика участия в реальных чатах
CREATE TABLE real_chat_participation (
    id INTEGER PRIMARY KEY,
    account_id INTEGER NOT NULL,
    chat_username TEXT NOT NULL,
...
| Файл | Описание |
|------|----------|
| `chat_context_analyzer.py` | LLM-анализ контекста чата |
| `real_chat_engine.py` | Координатор участия в реальных чатах |
| `tests/test_phase2_real_chats.py` | Тесты для Phase 2 |

### Обновления LLM промпта

```
💬 ГРУППЫ ДЛЯ АКТИВНОГО УЧАСТИЯ:
- @example_group: Москва Дизайнеры (релевантность: 0.9) [осталось 3 сообщ. сегодня]

9. "reply_in_chat" - Ответить на сообщение в публичной группе
   - Params: chat_username (обязательно), reply_text (опционально)
   - Доступно со стадии 8+
   - Система автоматически проанализирует контекст
```

### Безопасность Phase 2

- **Daily limits**: Max 3 сообщений на группу в день
- **Min stage**: Только аккаунты со stage 8+ могут писать в реальные чаты
- **Context analysis**: LLM проверяет уместность ответа
- **Confidence threshold**: Минимум 0.6 для отправки
- **Spam filtering**: Проверка генерируемого текста на spam-паттерны
- **Admin API status**: Только аккаунты со status=0 могут писать в реальные публичные чаты

### Лимиты на вступление в supergroups (2026-01-08)

Ограничения на количество supergroups по стадиям прогрева:

| Стадия | Макс. supergroups | Описание |
|--------|-------------------|----------|
| 1-9    | 0                 | Нельзя вступать в supergroups |
| 10     | 5                 | Начало вступления |
| 11     | 7                 | Постепенное увеличение |
| 12     | 10                | Средний лимит |
| 13     | 12                | Приближение к максимуму |
| 14     | 15                | Максимальный лимит |

**Реализация:**
- `config.py`: `CHAT_LIMITS["supergroups_by_stage"]` - лимиты по стадиям
- `database.py`: `can_join_supergroup(account_id, warmup_stage)` - проверка лимитов
- `executor.py`: проверка перед `join_channel` для supergroups

**Каналы** ограничены отдельно через `max_joins_per_day` (3) и `max_joins_per_session` (3).

```python
# Пример использования
from database import can_join_supergroup

can_join, reason = can_join_supergroup(account_id=34, warmup_stage=10)
# can_join=True, reason="Can join (2/5 supergroups at stage 10)"

can_join, reason = can_join_supergroup(account_id=34, warmup_stage=8)
# can_join=False, reason="Account at stage 8, need stage 10 to join supergroups"
```
...
# Результаты (2026-01-08):
# ✅ Database Functions - PASS
# ✅ ChatContextAnalyzer - PASS
# ✅ LLM Prompt - PASS
