...
## Фаза 1: Контролируемая среда (ВЫСОКИЙ ПРИОРИТЕТ)
...
```
HELPER АККАУНТЫ:
[ ] Синхронизация с Admin API
[ ] Флаги can_initiate_dm работают
[ ] Персоны генерируются для helpers

ЛИЧНЫЕ ДИАЛОГИ:
[ ] Warmup может начать диалог с helper
[ ] Helper не может начать диалог
[ ] Сообщения отправляются корректно
[ ] История сохраняется
[ ] Паузы между сообщениями естественные
[ ] Диалоги завершаются естественно
[ ] LLM генерирует релевантные ответы

ПРИВАТНЫЕ ГРУППЫ:
[ ] Группы создаются в Telegram
...
ИНТЕГРАЦИЯ:
[ ] ConversationEngine работает в scheduler
[ ] GroupEngine работает в scheduler
[ ] Нет конфликтов с основным прогревом
