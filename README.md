...
├── scheduler.py               # Автоматический планировщик прогрева
...
├── persona_agent.py           # Генератор персон для аккаунтов
├── search_agent.py            # Поиск релевантных каналов
├── auth.py                    # API токены и авторизация
├── data/
│   ├── sessions.db            # База данных SQLite
│   ├── channels_data.json     # Каналы из TGStat
│   ├── bots_data.json         # Список ботов
│   └── api_tokens.json        # API токены
├── docs/
...
├── tests/                     # Тесты
├── logs/                      # Логи (heat_up.log)
├── .agent/workflows/          # Правила для AI-агентов
...
- [x] API с токен-авторизацией
- [x] Автоматический scheduler
- [x] Персоны для аккаунтов
- [x] Поиск релевантных каналов
